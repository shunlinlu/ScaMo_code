...
def save_joint(f, anim, names, t, i, order='zyx', positions=False):
    ...
    predefined_end_site = {'LeftToe': [0.0026358, -0.0055791, 0.0119288], 'RightToe': [-0.0025372, -0.0048144, 0.0123348], 'Head': [0, 0, 0], 'LeftHand': [0.0760238, 0.001885, -0.0101412], 'RightHand': [-0.0756476, 0.004791, -0.011853]}
    f.write("%sJOINT %s\n" % (t, names[i]))
    f.write("%s{\n" % t)
    t += '\t'
    ...
    if end_site:
        ...
        t += '\t'
        # f.write("%sOFFSET %f %f %f\n" % (t, 0.0, 0.0, 0.0))
        f.write("%sOFFSET %f %f %f\n" % (t, predefined_end_site[names[i]][0], predefined_end_site[names[i]][1], predefined_end_site[names[i]][2]))
        t = t[:-1]
        f.write("%s}\n" % t)
