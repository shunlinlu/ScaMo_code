...
class LFQ(Module):
    ...
    def forward(
        ...
        with quantization_context():
            ...
            quantized = torch.where(x > 0, codebook_value, -codebook_value) # (bs, 16, 1, 10) all 1 or -1

            # calculate indices
            # import pdb; pdb.set_trace()
            indices = reduce((quantized > 0).int() * self.mask.int(), 'b n c d -> b n c', 'sum') # (bs, 16, 1)
        ...
        # returns
        ret = (x, indices, aux_loss, entropy_aux_loss, torch.tensor(indices.unique().numel() / self.codebook_size))
        # import pdb; pdb.set_trace()
